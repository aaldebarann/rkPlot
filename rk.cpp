...
std::vector<step> solveIvp(double (*f)(double , double), double x0, double u0, double eps, double xMax, double borderEps,
    ...
    if(method == "RK2") {
        ...
        throw std::invalid_argument("Unknow method. Available methods: \"RK2\", \"RK4\".");
    }
...
std::pair<double, Vec3> RK4_sys3(Vec3 (*f)(double, Vec3), double x0, Vec3 u0, double h) {
    ...
    xx = x + h;
    Vec3 k1 = f(x, v);
    Vec3 k2 = f(x + h / 2, v + k1 * h / 2);
    Vec3 k3 = f(x + h / 2, v + k2 * h / 2);
    Vec3 k4 = f(x + h, v + k3 * h);
    vv = v + (k1 + k2 * 2 + k3 * 2 + k4) * h / 6;
    return {xx, vv};
}
std::vector<step_sys3> solveIvp(Vec3 (*f)(double , Vec3), double x0, Vec3 u0, double eps, double xMax, double borderEps,
                           const std::string& method, bool variableStep, double h) {

    std::pair<double, Vec3> (*RK)(Vec3 (*f)(double, Vec3), double, Vec3, double);
    size_t order;
    if(method == "RK4") {
        RK = RK4_sys3;
        order = 4;
    } else {
        throw std::invalid_argument("Unknow method. Available methods: \"RK4\".");
    }
    double le = 0; // локальная погрешность
    size_t C1 = 0, C2 = 0; // счетчики деления шага и удвоения шага
    std::vector<step_sys3> result;
    result.emplace_back(x0, u0, h, le, u0, 0, 0);
    // координаты последней посчитанной точки
    double x = x0;
    Vec3 v = u0;
    std::pair<double, Vec3> p1, p2, p3;
    while(xMax - x > borderEps) {
        p1 = RK(f, x, v, h); // делаем шаг из (x, v)
        p2 = RK(f, x, v, h/2); // делаем половинный шаг из (x, v)
        p3 = RK(f, p2.first, p2.second, h/2); // делаем половинный шаг из (x_+1/2, v_+1/2)

        Vec3 s = (p1.second - p3.second) / ((1 << order) - 1.0);
        le = s.norm() * (1 << order);
